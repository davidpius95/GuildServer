...
const buildOpenUrl = (domain: string) => {
  if (
    /(\.localhost|\.local|\.test|\.example)$/i.test(domain) ||
    /^localhost$/i.test(domain) ||
    /^127\.0\.0\.1$/.test(domain) ||
    /^(10\.|192\.168\.|172\.(1[6-9]|2\d|3[0-1])\.)/.test(domain)
  ) {
    return `http://${domain}`
  }
...
function DomainRow({ domain, appId, isPrimary, removeDomain, setPrimaryDomain, verifyDomain }: any) {
  const [copiedField, setCopiedField] = useState<string | null>(null);
  const [selectedRegistrar, setSelectedRegistrar] = useState<string>("");
  
  const instructionsQuery = trpc.domain.getInstructions.useQuery(
    { id: domain.id, applicationId: appId },
    { enabled: !domain.verified && !domain.isAutoGenerated }
  );
  ...
  const activeRegistrar = instructions?.registrars.find((r: any) => r.name === selectedRegistrar) || instructions?.registrars[0];
  const domainUrl = buildOpenUrl(domain.domain)
  const canOpenDomain = domain.verified || domain.status === "active" || domain.isAutoGenerated

  useEffect(() => {
    if (instructions && !selectedRegistrar && instructions.registrars.length > 0) {
      setSelectedRegistrar(instructions.registrars[0].name);
    }
  }, [instructions, selectedRegistrar]);
  ...
  return (
    <div className="flex flex-col gap-3 p-3 rounded-lg border">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <code className="text-sm font-semibold">{domain.domain}</code>
            {isPrimary && <Badge variant="default" className="text-xs">Primary</Badge>}
            {domain.isAutoGenerated && <Badge variant="outline" className="text-xs">Auto</Badge>}
          ...
          {domain.verificationError && (
             <p className="text-xs text-red-500 mt-1">Verification failed: {domain.verificationError}</p>
          )}
        </div>
        <div className="flex flex-wrap justify-end gap-1">
          ...
          {!isPrimary && domain.verified && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => setPrimaryDomain.mutate({ id: domain.id, applicationId: appId })}
            >
              Set Primary
            </Button>
          )}
          <Button
            ...
            onClick={() => removeDomain.mutate({ id: domain.id, applicationId: appId })}
          >
      ...
      {!domain.verified && !domain.isAutoGenerated && instructions && (
        <div className="mt-2 bg-muted/30 p-4 rounded-md border border-muted">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-sm font-medium">
              {instructions.method === "dns" ? "DNS Setup" : "URL Forwarding Setup"}
            </h4>
            <Select value={selectedRegistrar} onValueChange={setSelectedRegistrar}>
              ...
              <SelectContent>
                {instructions.registrars.map((r: any) => (
          ...
          {/* What's happening + what the user needs to do */}
          {instructions.description && (
            <p className="text-xs text-muted-foreground mb-3 leading-relaxed">{instructions.description}</p>
          )}
          ...
          {instructions.record && (
            <div className="mb-3">
              ...
              <div className="grid grid-cols-[auto_1fr] sm:grid-cols-3 gap-2">
                {([
                  ["Type", instructions.record.type, "type"],
                  ["Host / Name", instructions.record.name, "name"],
                  ["Value", instructions.record.value, "value"],
                ] as const).map(([label, value, field]) => (
                  <div key={field}>
                    <span className="text-[10px] uppercase text-muted-foreground font-semibold mb-1 block">{label}</span>
                    <div className="flex items-center gap-2 bg-background border rounded px-2 py-1.5">
                      ...
                      <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => copyToClipboard(value, field)}>
                        {copiedField === field ? <CheckCircle className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3" />}
                      </Button>
                    </div>
                  </div>
                ))}
          ...
          {activeRegistrar && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase text-muted-foreground font-semibold">Steps for {activeRegistrar.name}</span>
                {activeRegistrar.docUrl && (
                  <a
                    href={activeRegistrar.docUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-xs text-primary hover:underline"
                  >
                    <ExternalLink className="h-3 w-3" />
                    {activeRegistrar.name} docs
                  </a>
                )}
              </div>
              <ol className="list-decimal list-inside text-xs text-muted-foreground space-y-2">
                {activeRegistrar.steps.map((step: string, i: number) => (
                  <li key={i} dangerouslySetInnerHTML={{ __html: step.replace(/`([^`]+)`/g, '<code class="bg-muted px-1 py-0.5 rounded text-foreground font-mono">$1</code>') }} />
                ))}
              </ol>
              {activeRegistrar.notes && (
                <p className="text-[11px] text-amber-600 dark:text-amber-500">{activeRegistrar.notes}</p>
              )}
              ...
              {instructions.generic && (
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 bg-background border rounded px-2 py-1.5">
                      <code className="text-xs flex-1 truncate">{instructions.generic.from}</code>
                      <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => copyToClipboard(instructions.generic.from, "from")}>
                        {copiedField === "from" ? <CheckCircle className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3" />}
                      </Button>
                    </div>
                  </div>
                  <div className="flex-1">
                    <span className="text-[10px] uppercase text-muted-foreground font-semibold mb-1 block">To</span>
                    <div className="flex items-center gap-2 bg-background border rounded px-2 py-1.5">
                      <code className="text-xs flex-1 truncate">{instructions.generic.to}</code>
                      <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => copyToClipboard(instructions.generic.to, "to")}>
                        {copiedField === "to" ? <CheckCircle className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3" />}
                      </Button>
                    </div>
                  </div>
                </div>
...
export default function ApplicationDetailPage() {
  ...
  const [domainMethod, setDomainMethod] = useState<"dns" | "redirect">("dns")
  ...
  const verifyDomain = trpc.domain.verify.useMutation({
    onSuccess: (data: any) => {
      utils.domain.list.invalidate()
      if (!data?.verified) {
        toast.error(data?.domain?.verificationError || "Could not verify this domain yet.")
        return
      }
      if (data?.needsRedeploy) {
        toast.success("Domain verified! Redeploy to start serving it.", {
          duration: 8000,
          action: {
            label: "Redeploy",
  ...
  const selectedDeploy = selectedDeployment
    ...
    : deploymentsList[0]
  const selectedDeploymentUrl =
    extractUrl(selectedDeploy?.deploymentLogs) ||
    extractUrl(selectedDeploy?.buildLogs) ||
    deploymentStream.accessUrl ||
    deploymentStream.directUrl ||
    null
  const selectedDeploymentDuration = selectedDeploy?.completedAt
    ? formatDuration(selectedDeploy.createdAt, selectedDeploy.completedAt)
    : selectedDeploy?.createdAt
      ? formatDuration(selectedDeploy.createdAt)
      : null
  const currentPhase = deploymentStream.phases[deploymentStream.phases.length - 1]
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    ...
                    <br />
                    <span className="text-xs italic opacity-80">Connect via DNS to keep your own domain in the address bar with its own SSL certificate, or use URL forwarding to redirect to your GuildServer address.</span>
                  </CardDescription>
                </div>
                <Button
                  variant="outline"
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border-t pt-4">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <button
                    type="button"
                    onClick={() => setDomainMethod("dns")}
                    className={`text-left rounded-lg border p-3 transition-colors ${domainMethod === "dns" ? "border-primary bg-primary/5" : "border-muted hover:border-muted-foreground/30"}`}
                  >
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Your domain stays in the address bar with its own SSL certificate (CNAME / A record).</p>
                  </button>
                  <button
                    type="button"
                    onClick={() => setDomainMethod("redirect")}
                    className={`text-left rounded-lg border p-3 transition-colors ${domainMethod === "redirect" ? "border-primary bg-primary/5" : "border-muted hover:border-muted-foreground/30"}`}
                  >
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <Link2 className="h-3.5 w-3.5" />
                      URL forwarding
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">Registrar redirects visitors to your GuildServer address (no DNS changes needed).</p>
                  </button>
                </div>
        ...
        <TabsContent value="build-logs">
          <Card>
            ...
            <CardContent>
              {(deploymentStream.status || selectedDeploy || selectedDeploymentUrl) && (
                  ...
                  <div className="rounded-xl border bg-muted/30 p-3">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {deploymentStream.phases.length > 0
                        ? `${deploymentStream.phases.length} stage${deploymentStream.phases.length === 1 ? "" : "s"} tracked`
                        : "Stage timing appears as the deploy progresses"}
                    </p>
                  </div>
                  <div className="rounded-xl border bg-muted/30 p-3">
                    <p className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">URL</p>
                    {selectedDeploymentUrl ? (
                      <div className="mt-2 flex flex-wrap gap-2">
                        <Button size="sm" asChild className="gap-1">
                          <a href={selectedDeploymentUrl} target="_blank" rel="noopener noreferrer">
                            <ExternalLink className="h-3 w-3" />
                            Open
                          </a>
                        </Button>
                        <Button
                          ...
                          variant="outline"
                          className="gap-1"
                          onClick={() => copyToClipboard(selectedDeploymentUrl, "deployment-url")}
                        >
                          {copiedField === "deployment-url" ? (
                            <CheckCircle className="h-3 w-3 text-green-500" />
                          ) : (
                            <Copy className="h-3 w-3" />
                          )}
              ...
              {!deploymentStream.accessUrl && selectedDeploymentUrl && (
                <div className="mt-4 p-3 bg-green-950/30 border border-green-800/30 rounded-lg">
                  <p className="text-sm text-green-400 font-medium">Deployment URL:</p>
                  <a
                    href={selectedDeploymentUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-blue-400 hover:underline flex items-center gap-1 mt-1"
                  >
