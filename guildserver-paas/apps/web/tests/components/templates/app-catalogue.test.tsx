...
describe('App catalog', () => {
  it('searches purpose labels and clears an empty result', () => {
    ...
    expect(screen.getByRole('button', { name: 'Configure Next.js' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Configure PostgreSQL' })).not.toBeInTheDocument()
    ...
    expect(screen.getByRole('button', { name: 'Configure PostgreSQL' })).toBeInTheDocument()
  })
  ...
  it('exposes selected quick filters and removes recommendations while filtering', () => {
    ...
    expect(screen.getByRole('button', { name: 'Configure PostgreSQL' })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Configure Next.js' })).not.toBeInTheDocument()
  })
})
