...
- Node environments set to `production` and App/API URLs mapped to `https://guild-technologies.com`.
...
- DNS `CNAME` records were updated in Cloudflare to route traffic from `guild-technologies.com` and `www.guild-technologies.com` through the tunnel.
...
When users deployed new projects (e.g., `gbca.guild-technologies.com`), the generated URLs were initially inaccessible. Several networking layers were reconfigured to fix this:

- **Wildcard DNS**: Added a CNAME record for `*.guild-technologies.com` pointing to the Cloudflare Tunnel, ensuring any subdomains automatically hit the server.
- **Tunnel Wildcard Ingress**: Updated `/etc/cloudflared/config.yml` to route `*.guild-technologies.com` to Traefik (`http://localhost:80`).
...
4. **Homepage URL**: `https://guild-technologies.com`
5. **Authorization callback URL**: `https://guild-technologies.com/api/auth/callback/github`
...
5. **Authorized JavaScript origins**: `https://guild-technologies.com`
6. **Authorized redirect URIs**: `https://guild-technologies.com/api/auth/callback/google`
...
If users need to link their GitHub repositories for automatic deployments, a **GitHub App** or Webhook must be configured pointing to `https://guild-technologies.com/webhooks/github`.
...
If you deploy a brand new app (e.g. `newapp.guild-technologies.com`) and your browser says "Server Not Found", but the logs say it deployed successfully, it is likely your Mac's DNS cache holding onto a stale "Not Found" result.
